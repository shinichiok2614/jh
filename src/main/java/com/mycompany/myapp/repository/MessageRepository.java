...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query(
        value = "select message from Message message left join fetch message.messagelist",
        countQuery = "select count(message) from Message message"
    )
    Page<Message> findAllWithToOneRelationships(Pageable pageable);

    @Query("select message from Message message left join fetch message.messagelist")
    List<Message> findAllWithToOneRelationships();

    @Query("select message from Message message left join fetch message.messagelist where message.id =:id")
