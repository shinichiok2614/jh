  ...
  return (
    <div>
      <h2 id="message-heading" data-cy="MessageHeading">
        <Translate contentKey="jhSeaportApp.message.home.title">Messages</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhSeaportApp.message.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/message/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhSeaportApp.message.home.createLabel">Create new Message</Translate>
          </Link>
        </div>
      </h2>
        ...
        {messageList && messageList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhSeaportApp.message.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="jhSeaportApp.message.sender">Sender</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="jhSeaportApp.message.messagelist">Messagelist</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {messageList.map((message, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`jhSeaportApp.Status.${message.status}`} />
                  </td>
                  <td>{message.sender ? <Link to={`/person/${message.sender.id}`}>{message.sender.id}</Link> : ''}</td>
                  <td>
                    {message.messagelist ? <Link to={`/message-list/${message.messagelist.id}`}>{message.messagelist.name}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhSeaportApp.message.home.notFound">No Messages found</Translate>
            </div>
          )
        )}
      </div>
